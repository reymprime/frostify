<script>
  // ── Frostify Icon System ──────────────────────────────────
  // Inline SVG icons (Lucide-style) — walang external dependency,
  // walang emoji. Usage: <Icon name="play" size={20} />
  let { name, size = 20, filled = false, class: cls = '' } = $props()
  ...
  const filledIcons = new Set(['play', 'pause'])
  let useFill = $derived(filled || filledIcons.has(name))
</script>
