...
<div class="px-5">
      ...
      {#each $playlists as pl (pl.id)}
        <div class="glass rounded-2xl p-3">
          <div class="flex items-center gap-3">
            ...
            >
              <div class="bg-glacier text-frost grid h-11 w-11 shrink-0 place-items-center rounded-xl text-lg">♫</div>
              <div class="min-w-0">
            ...
            {#if pl.tracks.length}
              <button
                class="bg-frost text-ink grid h-9 w-9 shrink-0 place-items-center rounded-full text-sm font-bold"
          ...
          {#if openId === pl.id}
            <div class="mt-3 space-y-2">
              {#if $currentTrack}
                <button
                  class="glass text-frost w-full rounded-xl p-2.5 text-sm font-semibold"
                  onclick={() => addToPlaylist(pl.id, $currentTrack)}
                >
                  ＋ Add current track ({$currentTrack.title})
                </button>
              {/if}
  ...
  {:else}
    <p class="text-mist pb-4 text-sm">Tap ♡ on any track to save it here.</p>
  {/if}
</div>
